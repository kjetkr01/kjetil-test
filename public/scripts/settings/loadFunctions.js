...
async function loadDefaultPage(setting) {
    ...
    const PPInfoBtn = `<button class="settingsButton pointer" style="padding:0;" onClick="showPublicProfileInfo();">${infoSVG}</button>`;
    if (settings.publicprofile === true) {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Offentlig profil ${PPInfoBtn}`, true, "publicprofile", "spacingTop");
    } else {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Offentlig profil ${PPInfoBtn}`, false, "publicprofile", "spacingTop");
    }

    const DLInfoBtn = `<button class="settingsButton pointer" style="padding:0;" onClick="showDisplayLeaderboardsInfo();">${infoSVG}</button>`;
    if (settings.displayleaderboards === true) {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Ledertavler synlighet ${DLInfoBtn}`, true, "displayleaderboards");
    } else {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Ledertavler synlighet ${DLInfoBtn}`, false, "displayleaderboards");
    }

    const DWLInfoBtn = `<button class="settingsButton pointer" style="padding:0;" onClick="showDisplayWorkoutListInfo();">${infoSVG}</button>`;
    if (settings.displayworkoutlist === true) {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Trener i dag listen synlighet ${DWLInfoBtn}`, true, "displayworkoutlist");
    } else {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Trener i dag listen synlighet ${DWLInfoBtn}`, false, "displayworkoutlist");
    }

    /*
    const DSInfoBtn = `<button class="settingsButton pointer" style="padding:0;" onClick="showDisplayStatisticsInfo();">${infoSVG}</button>`;
    if (settings.displaystatistics === true) {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Bruk dataene mine til statistikk ${DSInfoBtn}`, true, "displaystatistics");
    } else {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Bruk dataene mine til statistikk ${DSInfoBtn}`, false, "displaystatistics");
    }
    */

    const AEACInfoBtn = `<button class="settingsButton pointer" style="padding:0;" onClick="showExternalAPIRequestsInfo();">${infoSVG}</button>`;
    if (settings.externalapirequests === true) {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Tillat API forespørsler ${AEACInfoBtn}`, true, "externalapirequests");
    } else {
        settingsGrid.innerHTML += getTemplateWithCheckbox(`Tillat API forespørsler ${AEACInfoBtn}`, false, "externalapirequests");
    }
